...
for j in range(len(x_con)):
    ...
    result = pn.binarize(pn.Track(pianoroll=bolo*100, program=0, is_drum=False,name='my awesome piano'),threshold=0.5)
    multitracksam11 = pn.Multitrack(tracks=[result])
    pn.write(multitracksam11,'./result/20_' + sys.argv[j+1].split('_')[0] + '.mid')
