...
best = 100.0
model = Sequential()
model.add(Bidirectional(LSTM(128,input_shape = (seq_len,128),recurrent_activation='sigmoid',return_sequences=True,activation='sigmoid',stateful=True),batch_input_shape = (1,seq_len,128)))
model.add(Bidirectional(LSTM(128,recurrent_activation='sigmoid',activation='sigmoid',stateful=True)))
...
model.add(Bidirectional(LSTM(64,recurrent_activation='sigmoid',activation='sigmoid',return_sequences=True,stateful=True)))     
...
for i in range(20):
    ...
    jojo = np.array(kolo)
    for j in range(5):
        r2 = pn.Track(pianoroll=jojo[j]*300, program=0, is_drum=False,name='my awesome piano')
